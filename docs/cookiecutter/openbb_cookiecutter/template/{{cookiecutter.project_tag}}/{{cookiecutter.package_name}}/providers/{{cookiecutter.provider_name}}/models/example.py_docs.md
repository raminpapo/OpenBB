# Documentation: cookiecutter/openbb_cookiecutter/template/{{cookiecutter.project_tag}}/{{cookiecutter.package_name}}/providers/{{cookiecutter.provider_name}}/models/example.py

## File Metadata
- **Path**: `cookiecutter/openbb_cookiecutter/template/{{cookiecutter.project_tag}}/{{cookiecutter.package_name}}/providers/{{cookiecutter.provider_name}}/models/example.py`
- **Size**: 3,585 characters, 122 lines
- **Words**: 415
- **Extension**: .py
...
**Classes** (6):
`ExampleQueryParams`, `to`, `ExampleData`, `ExampleFetcher`, `is`, `needs`

**Functions** (3):
`transform_query`, `extract_data`, `transform_data`

**Imports** (12):
`the`, `a`, `typing`, `Any`, `openbb_core.provider.abstract.data`, `Data`, `openbb_core.provider.abstract.fetcher`, `Fetcher`, `openbb_core.provider.abstract.query_params`, `QueryParams`, `pydantic`, `Field`


## Key Components

**Class `ExampleQueryParams`**: Example provider query.

    This is the definition of our query parameters that are specific to this provider.
    We use this class to create our own parameters that will provided as input to the
  

**Class `ExampleData`**: Sample provider data.

    The fields are displayed as-is in the output of the command. In this case, its the
    Open, High, Low, Close and Volume data.

**Class `ExampleFetcher`**: Example Fetcher class.

    This class is responsible for the actual data retrieval.

**Class `needs`**: No documentation
...
## Notes
- Generated: 2025-11-18T07:54:34.758834
