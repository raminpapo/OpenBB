# Documentation: cli/openbb_cli/controllers/base_controller.py

## File Metadata
- **Path**: `cli/openbb_cli/controllers/base_controller.py`
- **Size**: 33,452 characters, 917 lines
- **Words**: 2,637
- **Extension**: .py
...
**Classes** (4):
`BaseController`, `for`, `for`, `to`

**Functions** (22):
`choices_default`, `__init__`, `update_completer`, `check_path`, `load_class`, `save_class`, `custom_reset`, `print_help`, `parse_input`, `switch`, `call_cls`, `call_home`, `call_help`, `call_quit`, `call_exit`, `call_reset`, `call_record`, `call_stop`, `call_results`, `parse_simple_args`

**Imports** (34):
`argparse`, `difflib`, `os`, `re`, `shlex`, `abc`, `ABCMeta`, `datetime`, `datetime`, `pathlib`, `Path`, `typing`, `Any`, `pandas`, `openbb_cli.config.completer`, `NestedCompleter`, `openbb_cli.config.constants`, `SCRIPT_TAGS`, `openbb_cli.controllers.choices`, `build_controller_choice_map`


## Key Components

**Class `BaseController`**: Base class for a cli controller.

**Class `for`**: No documentation

**Class `to`**: Implement custom reset.
...
## Notes
- Generated: 2025-11-18T07:54:34.644222
