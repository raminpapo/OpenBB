...
- **Functions** (32): remove_file, print_goodbye, bootup, welcome_message, reset, suppress_stdout, first_time_user, parse_and_split_input, return_colored_value, print_rich_table ... and 22 more
- **Import Statements**: 5
...
- **`bootup()`**
- **`welcome_message()`**
- **`reset(queue: list[str] | None = None)`**
- **`suppress_stdout()`**
- **`return_colored_value(value: str)`**
- **`print_rich_table(  # noqa: PLR0912
    df: pd.DataFrame,
    show_index: bool = False,
    title: str = "",
    index_name: str = "",
...
- **`check_file_type_saved(valid_types: list[str] | None = None)`**
- **`save_to_excel(df, saved_path, sheet_name, start_row=0, index=True, header=True)`**
- **`system_clear()`**
...
- ⚠️ Uses `os.system()` - potential command injection risk
- ⚠️ Uses `input()` - validate user input


---

**Generated**: 2025-11-19T02:16:44.900496Z
