# Documentation: openbb_platform/providers/tmx/openbb_tmx/models/calendar_earnings.py

## File Metadata
- **Path**: `openbb_platform/providers/tmx/openbb_tmx/models/calendar_earnings.py`
- **Size**: 5,347 characters, 148 lines
- **Words**: 443
- **Extension**: .py
...
**Classes** (3):
`TmxCalendarEarningsQueryParams`, `TmxCalendarEarningsData`, `TmxCalendarEarningsFetcher`

**Functions** (5):
`percent_validate`, `transform_query`, `aextract_data`, `create_task`, `transform_data`

**Imports** (22):
`datetime`, `datetime`, `typing`, `Any`, `openbb_core.provider.abstract.fetcher`, `Fetcher`, `openbb_core.provider.standard_models.calendar_earnings`, `pydantic`, `Field`, `the`, `datetime`, `timedelta`, `the`, `asyncio`, `json`, `openbb_tmx.utils`, `gql`, `openbb_tmx.utils.helpers`, `get_data_from_gql`, `pandas`


## Key Components

**Class `TmxCalendarEarningsQueryParams`**: TMX Calendar Earnings Query.

**Class `TmxCalendarEarningsData`**: TMX Calendar Earnings Data.

**Class `TmxCalendarEarningsFetcher`**: Transform the query, extract and transform the data from the TMX endpoints.
...
## Notes
- Generated: 2025-11-18T07:54:41.768264
