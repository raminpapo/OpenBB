# File Documentation: test_tmx_fetchers.py

## Metadata
- **Path**: `openbb_platform/providers/tmx/tests/test_tmx_fetchers.py`
- **Size**: 8,931 bytes
- **Lines**: 282
- **Category**: python
- **Extension**: .py
...
This is a **python** file named `test_tmx_fetchers.py`.

**Python Module**

- **Functions** (24): vcr_config, test_tmx_equity_profile_fetcher, test_tmx_equity_search_fetcher, test_tmx_available_indices_fetcher, test_tmx_calendar_earnings_fetcher, test_tmx_company_filings_fetcher, test_tmx_company_news_fetcher, test_tmx_equity_historical_fetcher, test_tmx_equity_quote_fetcher, test_tmx_etf_countries_fetcher ... and 14 more
- **Import Statements**: 1
...
- **`vcr_config()`**
- **`test_tmx_equity_profile_fetcher(credentials=test_credentials)`**
- **`test_tmx_equity_search_fetcher(credentials=test_credentials)`**
- **`test_tmx_available_indices_fetcher(credentials=test_credentials)`**
- **`test_tmx_calendar_earnings_fetcher(credentials=test_credentials)`**
- **`test_tmx_company_filings_fetcher(credentials=test_credentials)`**
- **`test_tmx_company_news_fetcher(credentials=test_credentials)`**
- **`test_tmx_equity_historical_fetcher(credentials=test_credentials)`**
- **`test_tmx_equity_quote_fetcher(credentials=test_credentials)`**
- **`test_tmx_etf_countries_fetcher(credentials=test_credentials)`**
- **`test_tmx_etf_holdings_fetcher(credentials=test_credentials)`**
- **`test_tmx_etf_info_fetcher(credentials=test_credentials)`**
- **`test_tmx_etf_search_fetcher(credentials=test_credentials)`**
- **`test_tmx_etf_sectors_fetcher(credentials=test_credentials)`**
- **`test_tmx_gainers_fetcher(credentials=test_credentials)`**
- **`test_tmx_historical_dividends_fetcher(credentials=test_credentials)`**
- **`test_tmx_index_constituents_fetcher(credentials=test_credentials)`**
- **`test_tmx_index_sectors_fetcher(credentials=test_credentials)`**
- **`test_tmx_index_snapshots_fetcher(credentials=test_credentials)`**
- **`test_tmx_insider_trading_fetcher(credentials=test_credentials)`**
- **`test_tmx_options_chains_fetcher(credentials=test_credentials)`**
- **`test_tmx_price_target_consensus_fetcher(credentials=test_credentials)`**
- **`test_tmx_treasury_prices_fetcher(credentials=test_credentials)`**
- **`test_tmx_bond_prices_fetcher(credentials=test_credentials)`**
...
**Generated**: 2025-11-19T02:16:53.126001Z
