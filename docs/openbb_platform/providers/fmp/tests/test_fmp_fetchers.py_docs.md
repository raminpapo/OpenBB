# File Documentation: test_fmp_fetchers.py

## Metadata
- **Path**: `openbb_platform/providers/fmp/tests/test_fmp_fetchers.py`
- **Size**: 27,175 bytes
- **Lines**: 829
- **Category**: python
- **Extension**: .py
...
This is a **python** file named `test_fmp_fetchers.py`.

**Python Module**

- **Functions** (69): response_filter, vcr_config, test_fmp_company_filings_fetcher, test_fmp_crypto_historical_fetcher, test_fmp_currency_historical_fetcher, test_fmp_index_historical_fetcher, test_fmp_equity_historical_fetcher, test_fmp_company_news_fetcher, test_fmp_balance_sheet_fetcher, test_fmp_cash_flow_statement_fetcher ... and 59 more
- **Import Statements**: 2
...
- **`vcr_config()`**
- **`test_fmp_company_filings_fetcher(credentials=test_credentials)`**
- **`test_fmp_crypto_historical_fetcher(credentials=test_credentials)`**
- **`test_fmp_currency_historical_fetcher(credentials=test_credentials)`**
- **`test_fmp_index_historical_fetcher(credentials=test_credentials)`**
- **`test_fmp_equity_historical_fetcher(credentials=test_credentials)`**
- **`test_fmp_company_news_fetcher(credentials=test_credentials)`**
- **`test_fmp_balance_sheet_fetcher(credentials=test_credentials)`**
- **`test_fmp_cash_flow_statement_fetcher(credentials=test_credentials)`**
- **`test_fmp_income_statement_fetcher(credentials=test_credentials)`**
- **`test_fmp_available_indices_fetcher(credentials=test_credentials)`**
- **`test_fmp_key_executives_fetcher(credentials=test_credentials)`**
- **`test_fmp_world_news_fetcher(credentials=test_credentials)`**
- **`test_fmp_income_statement_growth_fetcher(credentials=test_credentials)`**
- **`test_fmp_balance_sheet_growth_fetcher(credentials=test_credentials)`**
- **`test_fmp_cash_flow_statement_growth_fetcher(credentials=test_credentials)`**
- **`test_fmp_share_statistics_fetcher(credentials=test_credentials)`**
- **`test_fmp_revenue_geographic_fetcher(credentials=test_credentials)`**
- **`test_fmp_revenue_business_line_fetcher(credentials=test_credentials)`**
- **`test_fmp_institutional_ownership_fetcher(credentials=test_credentials)`**
- **`test_fmp_insider_trading_fetcher(credentials=test_credentials)`**
- **`test_fmp_equity_ownership_fetcher(credentials=test_credentials)`**
- **`test_fmp_price_target_consensus_fetcher(credentials=test_credentials)`**
- **`test_fmp_price_target_fetcher(credentials=test_credentials)`**
- **`test_fmp_analyst_estimates_fetcher(credentials=test_credentials)`**
- **`test_fmp_historical_eps_fetcher(credentials=test_credentials)`**
- **`test_fmp_earnings_call_transcript_fetcher(credentials=test_credentials)`**
- **`test_fmp_historical_splits_fetcher(credentials=test_credentials)`**
- **`test_fmp_calendar_splits_fetcher(credentials=test_credentials)`**
...
**Generated**: 2025-11-19T02:16:50.714664Z
