# File Documentation: test_fred_fetchers.py

## Metadata
- **Path**: `openbb_platform/providers/fred/tests/test_fred_fetchers.py`
- **Size**: 16,412 bytes
- **Lines**: 532
- **Category**: python
- **Extension**: .py
...
This is a **python** file named `test_fred_fetchers.py`.

**Python Module**

- **Functions** (36): vcr_config, test_fredcpi_fetcher, test_fred_sofr_fetcher, test_fred_euro_short_term_rate_fetcher, test_fredsonia_fetcher, test_fred_ameribor_fetcher, test_fred_federal_funds_rate_fetcher, test_fredprojection_fetcher, test_frediorb_fetcher, test_fred_discount_window_primary_credit_rate_fetcher ... and 26 more
- **Import Statements**: 7
...
- **`vcr_config()`**
- **`test_fredcpi_fetcher(credentials=test_credentials)`**
- **`test_fred_sofr_fetcher(credentials=test_credentials)`**
- **`test_fred_euro_short_term_rate_fetcher(credentials=test_credentials)`**
- **`test_fredsonia_fetcher(credentials=test_credentials)`**
- **`test_fred_ameribor_fetcher(credentials=test_credentials)`**
- **`test_fred_federal_funds_rate_fetcher(credentials=test_credentials)`**
- **`test_fredprojection_fetcher(credentials=test_credentials)`**
- **`test_frediorb_fetcher(credentials=test_credentials)`**
- **`test_fred_discount_window_primary_credit_rate_fetcher(credentials=test_credentials)`**
- **`test_fred_european_central_bank_interest_rates_fetcher(
    credentials=test_credentials,
)`**
- **`test_fred_commercial_paper_fetcher(credentials=test_credentials)`**
- **`test_fred_spot_rate_fetcher(credentials=test_credentials)`**
- **`test_fred_high_quality_market_corporate_bond_fetcher(credentials=test_credentials)`**
- **`test_fred_treasury_constant_maturity_fetcher(credentials=test_credentials)`**
- **`test_fred_selected_treasury_constant_maturity_fetcher(credentials=test_credentials)`**
- **`test_fred_selected_treasury_bill_fetcher(credentials=test_credentials)`**
- **`test_fred_search_fetcher(credentials=test_credentials)`**
- **`test_fred_series_fetcher(credentials=test_credentials)`**
- **`test_fred_regional_fetcher(credentials=test_credentials)`**
- **`test_fred_balance_of_payments_fetcher(credentials=test_credentials)`**
- **`test_fred_yield_curve_fetcher(credentials=test_credentials)`**
- **`test_fred_retail_prices_fetcher(credentials=test_credentials)`**
- **`test_fred_bond_indices_fetcher(credentials=test_credentials)`**
- **`test_fred_mortgage_indices_fetcher(credentials=test_credentials)`**
- **`test_fred_university_of_michigan_fetcher(credentials=test_credentials)`**
- **`test_fred_senior_loan_officer_survey_fetcher(credentials=test_credentials)`**
- **`test_fred_survey_of_economic_conditions_chicago_fetcher(
    credentials=test_credentials,
)`**
- **`test_fred_manufacturing_outlook_texas_fetcher(
    credentials=test_credentials,
)`**
- **`test_fred_overnight_bank_funding_rate_fetcher(credentials=test_credentials)`**
...
**Generated**: 2025-11-19T02:16:51.066532Z
