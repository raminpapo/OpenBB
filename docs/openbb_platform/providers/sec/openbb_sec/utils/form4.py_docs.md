...
This is a **python** file named `form4.py`.

**Python Module**

- **Functions** (15): get_logger, setup_database, add_missing_column, compress_db, decompress_db, close_db, get_form_4_urls, clean_xml, get_form_4_data, response_callback ... and 5 more
...
- **`get_logger()`**
- **`setup_database(conn)`**
- **`add_missing_column(conn, column_name)`**
- **`compress_db(db_path)`**
- **`decompress_db(db_path)`**
- **`close_db(conn, db_path)`**
- **`get_form_4_urls(
...
- **`parse_form_4_data(  # noqa: PLR0915, PLR0912  # pylint: disable=too-many-branches
    data,
)`**
- **`download_data(urls, use_cache: bool = True)`**
- **`get_one(url)`**
- **`get_cached_data(urls, conn)`**
...
**Generated**: 2025-11-19T02:16:52.022091Z
