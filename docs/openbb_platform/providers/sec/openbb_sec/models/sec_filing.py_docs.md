# Documentation: openbb_platform/providers/sec/openbb_sec/models/sec_filing.py

## File Metadata
- **Path**: `openbb_platform/providers/sec/openbb_sec/models/sec_filing.py`
- **Size**: 26,824 characters, 716 lines
- **Words**: 2,204
- **Extension**: .py
...
**Classes** (4):
`SecFilingQueryParams`, `SecFilingData`, `SecBaseFiling`, `SecFilingFetcher`

**Functions** (26):
`base_url`, `name`, `cik`, `trading_symbols`, `sic`, `sic_organization_name`, `filing_date`, `period_ending`, `fiscal_year_end`, `document_type`, `has_cover_page`, `cover_page`, `description`, `document_urls`, `__init__`, `_adownload_file`, `download_file`, `try_html_table`, `_download_index_headers`, `document_to_dict`

**Imports** (52):
`datetime`, `date`, `typing`, `Any`, `openbb_core.app.model.abstract.error`, `OpenBBError`, `openbb_core.provider.abstract.data`, `Data`, `openbb_core.provider.abstract.fetcher`, `Fetcher`, `openbb_core.provider.abstract.query_params`, `QueryParams`, `pydantic`, `ConfigDict`, `openbb_core.provider.utils.helpers`, `run_async`, `openbb_sec.utils.helpers`, `cik_map`, `a`, `aiohttp_client_cache`


## Key Components

**Class `SecFilingQueryParams`**: SEC Filing Query Parameters.

**Class `SecFilingData`**: SEC Filing Data.

**Class `SecBaseFiling`**: No documentation

**Class `SecFilingFetcher`**: SEC Filing Fetcher.
...
## Notes
- Generated: 2025-11-18T07:54:40.614160
