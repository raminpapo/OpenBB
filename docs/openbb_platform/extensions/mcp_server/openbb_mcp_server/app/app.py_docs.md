# Documentation: openbb_platform/extensions/mcp_server/openbb_mcp_server/app/app.py

## File Metadata
- **Path**: `openbb_platform/extensions/mcp_server/openbb_mcp_server/app/app.py`
- **Size**: 29,097 characters, 779 lines
- **Words**: 2,281
- **Extension**: .py
...
**Functions** (26):
`_extract_brief_description`, `_get_mcp_config_from_route`, `_strip_api_prefix`, `_read_system_prompt_file`, `_build_runtime_middleware`, `create_mcp_server`, `in`, `customize_components`, `system_prompt_func`, `system_prompt_resource`, `in`, `in`, `available_categories`, `available_tools`, `activate_tools`, `deactivate_tools`, `list_prompts`, `execute_prompt`, `in`, `and`

**Imports** (65):
`asyncio`, `json`, `os`, `re`, `signal`, `sys`, `pathlib`, `Path`, `typing`, `Annotated`, `fastapi`, `FastAPI`, `fastapi.routing`, `APIRoute`, `fastmcp`, `FastMCP`, `fastmcp.prompts.prompt`, `FunctionPrompt`, `fastmcp.server.openapi`, `fastmcp.utilities.json_schema`


## Key Components

**Class `SSEShutdownWrapper`**: ASGI middleware to handle SSE connection shutdown gracefully.
...
## Notes
- Generated: 2025-11-18T07:54:36.183403
