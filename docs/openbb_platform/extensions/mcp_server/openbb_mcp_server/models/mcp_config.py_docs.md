# Documentation: openbb_platform/extensions/mcp_server/openbb_mcp_server/models/mcp_config.py

## File Metadata
- **Path**: `openbb_platform/extensions/mcp_server/openbb_mcp_server/models/mcp_config.py`
- **Size**: 8,950 characters, 272 lines
- **Words**: 915
- **Extension**: .py
...
**Classes** (5):
`MCPType`, `HTTPMethod`, `ArgumentDefinitionModel`, `PromptConfigModel`, `MCPConfigModel`

**Functions** (10):
`validate_name`, `validate_type`, `validate_content`, `validate_name`, `validate_tags`, `validate_methods`, `validate_config_consistency`, `to_dict`, `validate_mcp_config`, `is_valid_mcp_config`

**Imports** (12):
`re`, `enum`, `Enum`, `typing`, `Any`, `fastmcp.utilities.logging`, `get_logger`, `pydantic`, `BaseModel`, `this`, `exc`, `exc`


## Key Components

**Class `MCPType`**: Valid MCP type values.

**Class `HTTPMethod`**: Valid HTTP methods for route configuration.

**Class `ArgumentDefinitionModel`**: Model for validating prompt argument definitions.

**Class `PromptConfigModel`**: Model for validating individual prompt configurations.

**Class `MCPConfigModel`**: Model for validating the main MCP configuration structure.
...
## Notes
- Generated: 2025-11-18T07:54:36.188474
