# Documentation: openbb_platform/core/openbb_core/app/router.py

## File Metadata
- **Path**: `openbb_platform/core/openbb_core/app/router.py`
- **Size**: 17,598 characters, 519 lines
- **Words**: 1,332
- **Extension**: .py
...
**Classes** (7):
`from`, `OpenBBErrorResponse`, `Router`, `SignatureInspector`, `CommandMap`, `LoadingError`, `RouterLoader`

**Functions** (29):
`api_router`, `prefix`, `description`, `routers`, `__init__`, `command`, `command`, `command`, `include_router`, `get_attr`, `_search_attr`, `complete`, `polish_return_schema`, `validate_signature`, `inject_dependency`, `inject_return_annotation`, `get_description`, `get_operation_id`, `__init__`, `map`

**Imports** (34):
`traceback`, `warnings`, `collections.abc`, `Callable`, `functools`, `lru_cache`, `inspect`, `isclass`, `typing`, `fastapi`, `APIRouter`, `openbb_core.app.deprecation`, `DeprecationSummary`, `openbb_core.app.extension_loader`, `ExtensionLoader`, `openbb_core.app.model.abstract.warning`, `OpenBBWarning`, `openbb_core.app.model.example`, `filter_list`, `openbb_core.app.model.obbject`


## Key Components

**Class `from`**: OpenBB Error Response.

**Class `Router`**: OpenBB Router Class.

**Class `SignatureInspector`**: Inspect function signature.

**Class `CommandMap`**: Matching Routes with Commands.

**Class `LoadingError`**: Error loading extension.

**Class `RouterLoader`**: Router Loader.
