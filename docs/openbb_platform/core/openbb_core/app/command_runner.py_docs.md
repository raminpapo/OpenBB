# Documentation: openbb_platform/core/openbb_core/app/command_runner.py

## File Metadata
- **Path**: `openbb_platform/core/openbb_core/app/command_runner.py`
- **Size**: 24,232 characters, 647 lines
- **Words**: 1,968
- **Extension**: .py
...
**Classes** (5):
`from`, `ExecutionContext`, `ParametersBuilder`, `StaticCommandRunner`, `CommandRunner`

**Functions** (26):
`__init__`, `api_route`, `get_polished_parameter_list`, `get_polished_func`, `merge_args_and_kwargs`, `update_command_context`, `_warn_kwargs`, `_as_dict`, `validate_kwargs`, `build`, `_command`, `_chart`, `_extract_params`, `_execute_func`, `run`, `_trigger_command_output_callbacks`, `_extension_key`, `_clone_for_immutable`, `__init__`, `init_logging_service`

**Imports** (69):
`collections.abc`, `Callable`, `copy`, `deepcopy`, `dataclasses`, `asdict`, `datetime`, `datetime`, `inspect`, `Parameter`, `sys`, `exc_info`, `time`, `perf_counter_ns`, `typing`, `TYPE_CHECKING`, `warnings`, `catch_warnings`, `openbb_core.app.extension_loader`, `ExtensionLoader`
...
**Class `ParametersBuilder`**: Build parameters for a function.

**Class `StaticCommandRunner`**: Static Command Runner.

**Class `CommandRunner`**: Command runner.
...
## Notes
- Generated: 2025-11-18T07:54:35.411967
