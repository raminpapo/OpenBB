# Documentation: openbb_platform/core/openbb_core/app/static/package_builder.py

## File Metadata
- **Path**: `openbb_platform/core/openbb_core/app/static/package_builder.py`
- **Size**: 147,765 characters, 3,603 lines
- **Words**: 11,093
- **Extension**: .py
...
**Classes** (17):
`FileLock`, `PackageBuilder`, `ModuleBuilder`, `ImportDefinition`, `ClassDefinition`, `definition`, `definition`, `MethodDefinition`, `loader`, `DocstringGenerator`, `PathHandler`, `ReferenceGenerator`, `itself`, `provider_class`, `for`, `for`, `for`

**Functions** (73):
`create_indent`, `__init__`, `acquire`, `release`, `__init__`, `auto_build`, `build`, `_clean`, `_get_extension_map`, `_save_modules`, `_save_package`, `_save_reference_file`, `_run_linters`, `_write`, `_read`, `_diff`, `build`, `filter_hint_type_list`, `get_function_hint_type_list`, `get_path_hint_type_list`

**Imports** (145):
`builtins`, `contextlib`, `inspect`, `os`, `re`, `shutil`, `sys`, `textwrap`, `collections`, `OrderedDict`, `collections.abc`, `Callable`, `functools`, `partial`, `inspect`, `Parameter`, `json`, `dumps`, `pathlib`, `Path`


## Key Components

**Class `FileLock`**: Simple cross-platform file lock wrapper used only for this module.

**Class `PackageBuilder`**: Build the extension package for the Platform.

**Class `ModuleBuilder`**: Build the module for the Platform.

**Class `ImportDefinition`**: Build the import definition for the Platform.

**Class `ClassDefinition`**: Build the class definition for the Platform.

**Class `definition`**: No documentation

**Class `MethodDefinition`**: Build the method definition for the Platform.

**Class `loader`**: No documentation

**Class `DocstringGenerator`**: Dynamically generate docstrings for the commands.

**Class `PathHandler`**: Handle the paths for the Platform.

**Class `ReferenceGenerator`**: Generate the reference for the Platform.

**Class `itself`**: No documentation

**Class `provider_class`**: No documentation
...
## Notes
- Generated: 2025-11-18T07:54:35.486976
