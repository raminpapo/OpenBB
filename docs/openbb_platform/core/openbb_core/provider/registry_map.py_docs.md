# Documentation: openbb_platform/core/openbb_core/provider/registry_map.py

## File Metadata
- **Path**: `openbb_platform/core/openbb_core/provider/registry_map.py`
- **Size**: 8,227 characters, 208 lines
- **Words**: 673
- **Extension**: .py
...
**Classes** (5):
`from`, `RegistryMap`, `of`, `family`, `itself`

**Functions** (17):
`__init__`, `registry`, `available_providers`, `credentials`, `standard_extra`, `original_models`, `models`, `_get_credentials`, `_get_available_providers`, `_get_maps`, `_update_json_schema_extra`, `_get_models`, `_get_results_type`, `_extract_info`, `_get_model`, `_validate`, `_get_class_family`

**Imports** (22):
`copy`, `deepcopy`, `inspect`, `getfile`, `pathlib`, `Path`, `typing`, `Any`, `openbb_core.provider.abstract.data`, `Data`, `openbb_core.provider.abstract.fetcher`, `Fetcher`, `openbb_core.provider.abstract.query_params`, `QueryParams`, `openbb_core.provider.registry`, `Registry`, `pydantic`, `BaseModel`, `one`, `fetcher.`


## Key Components

**Class `from`**: Class to store information about providers in the registry.

**Class `of`**: Return the class family starting with the class itself until `object`.
...
## Notes
- Generated: 2025-11-18T07:54:35.520410
