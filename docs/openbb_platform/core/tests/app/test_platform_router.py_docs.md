# File Documentation: test_platform_router.py

## Metadata
- **Path**: `openbb_platform/core/tests/app/test_platform_router.py`
- **Size**: 5,174 bytes
- **Lines**: 213
- **Category**: python
- **Extension**: .py
...
This is a **python** file named `test_platform_router.py`.

**Python Module**

- **Classes** (1): MockBaseModel
- **Functions** (31): router, test_router_init, test_command, valid_function, test_include_router, router_loader, test_router_loader_init, test_from_extensions, signature_inspector, test_signature_inspector_init ... and 21 more
- **Import Statements**: 2
...
- **`router()`**
- **`test_router_init(router)`**
- **`test_command(router)`**
- **`test_include_router(router)`**
- **`router_loader()`**
- **`test_router_loader_init(router_loader)`**
- **`test_from_extensions(router_loader)`**
- **`signature_inspector()`**
- **`test_signature_inspector_init(signature_inspector)`**
- **`test_complete_signature(signature_inspector)`**
- **`test_complete_signature_error(signature_inspector)`**
- **`test_validate_signature(signature_inspector)`**
- **`test_inject_dependency(signature_inspector)`**
- **`test_get_description(signature_inspector)`**
- **`some_function()`**
- **`test_get_description_no_doc(signature_inspector)`**
- **`some_function()`**
- **`command_map()`**
- **`test_command_map_init(command_map)`**
- **`test_map(command_map)`**
- **`test_provider_coverage(command_map)`**
- **`test_command_coverage(command_map)`**
- **`test_get_command_map(command_map, router)`**
- **`test_get_provider_coverage(command_map, router)`**
- **`test_get_command_coverage(command_map, router)`**
- **`test_get_command(command_map)`**
...
**Generated**: 2025-11-19T02:16:46.639567Z
